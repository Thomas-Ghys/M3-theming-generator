...
export function addLightThemeTokens(destinationPathName: string, color: string) {
    ...
    if (color !== 'surface') {
        createThemeTokensFile(destinationPathName, color, templatesPath, 'base-light-color-tokens.txt');
        return;
    }
    
    createThemeTokensFile(destinationPathName, color, templatesPath, 'base-light-surface-tokens.txt');
}